...
The minimum viable contract and extended contract share the same core structure.

## Minimum Viable Fields

- `id` identifies the capability with semantic naming
- `version` tracks contract evolution using semantic versioning
- `owner` names the accountable team or individual
- `state` captures lifecycle stage
- `name` gives the capability a readable label
- `description` explains what the capability does and why it exists
- `inputs` names the declared inputs and the constraints that matter to the business
- `outputs` names the declared outputs and the guarantees they provide
